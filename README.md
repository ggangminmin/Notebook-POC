...
### ⚙️ 2026-01-02: AI 지침 설정 UI/UX 개선 + 시스템 프롬프트 오버라이드
...
#### 🔧 기술 구현
- [x] **systemPromptOverrides props**: ChatInterface → aiService 전달 체계 구축
- [x] **프롬프트 병합 로직**: 사용자 지침 + 기본 프롬프트 자동 결합
- [x] **문서 기반 정확성 강조**: 문서에 없는 내용 추측 금지 명시
...
- [ChatInterface.jsx:10](src/components/ChatInterface.jsx) - systemPromptOverrides props
- [aiService.js:410-430](src/services/aiService.js) - 프롬프트 오버라이드 로직

---

### 🎯 2026-01-02: 보라색 추론 배지 클릭 활성화 + 대화 기록 초기화

#### 💜 보라색 추론 배지 클릭 이벤트 활성화 (NEW!)
- [x] **클릭 가능한 추론 배지**: `[문서 맥락 기반 추론]` 같은 보라색 배지를 클릭할 수 있음
- [x] **상세 정보 표시**: 클릭 시 알림창으로 추론 내용 및 설명 제공
  - 💡 추론 배지 클릭됨
  - 내용: "문서 맥락 기반 추론"
  - AI가 전체 문서 맥락을 참고하여 생성한 인사이트
- [x] **호버 효과 추가**:
  - 배경색 변화 (`hover:bg-purple-200`)
  - 텍스트 색상 변화 (`hover:text-purple-900`)
  - 클릭 애니메이션 (`active:scale-95`)
- [x] **`<span>` → `<button>` 변환**: 접근성 향상 및 클릭 이벤트 최적화

**구현 위치**: [ChatInterface.jsx:259-273](src/components/ChatInterface.jsx) - 추론 배지 렌더링

#### 🧹 대화 기록 자동 복원/저장 비활성화
- [x] **깨끗한 초기 화면**: 앱 시작 시 빈 메시지 배열로 시작
- [x] **localStorage 자동 복원 제거**: 이전 대화 기록이 자동으로 불러와지지 않음
- [x] **localStorage 자동 저장 비활성화**: 대화 내용이 자동으로 저장되지 않음
- [x] **세션 기반 대화**: 브라우저 새로고침 시 대화 초기화
- [x] **대화 기록 유지**: 파일 추가/제거 시에도 기존 대화는 유지 (환영 메시지와 분석 메시지만 필터링)

**효과**:
- 처음 접속 시 깨끗한 빈 화면
- 불필요한 이전 대화 기록 제거
- 사용자가 원하는 시점부터 새로운 대화 시작 가능

**구현 위치**:
- [ChatInterface.jsx:11-12](src/components/ChatInterface.jsx) - 초기 상태
- [ChatInterface.jsx:19-31](src/components/ChatInterface.jsx) - localStorage 저장 비활성화

---

### 🎯 2026-01-02: 대괄호 없는 페이지 패턴 자동 감지 + .gitignore 개선

#### 📌 대괄호 없는 페이지 인용 자동 감지 (NEW!)
- [x] **자연어 페이지 참조 지원**: AI 응답에서 대괄호 없이도 페이지 번호 자동 감지
  - `페이지 15` → 클릭 가능한 배지로 자동 변환
  - `페이지 15 17` → 두 개의 배지 생성
  - `15-18` → 범위 배지 생성 (시작-끝)
  - `15 17 22` → 연속된 숫자를 각각 배지로 변환

- [x] **스마트 패턴 처리**:
  - "페이지" 접두사 패턴: `페이지\s+((?:\d+(?:\s+\d+)*))` 정규식
  - 범위 패턴: `\b(\d+)\s*-\s*(\d+)\b` 감지
  - 연속 숫자 패턴: `\b(\d+(?:\s+\d+)+)\b` 처리
  - 중복 패턴 방지: 이미 처리된 영역은 건너뛰기

- [x] **기존 대괄호 패턴과 병행 지원**:
  - `[5]`, `[5-8]`, `[5, 10]` 기존 패턴 완전 호환
  - `[35, 38, 문서 맥락 기반 추론]` 복합 인용 유지
  - 대괄호 있는 부분 먼저 처리 → 대괄호 없는 부분 후처리

- [x] **React Fragment 최적화**: 텍스트와 배지를 배열로 반환하여 ReactMarkdown과 완벽 호환
...
AI 응답: "자세한 내용은 15-18 범위를 참조하세요."
→ "자세한 내용은 [배지:15-18] 범위를 참조하세요." (클릭 시 페이지 15로 이동)
```

**구현 위치**: [ChatInterface.jsx:18-162](src/components/ChatInterface.jsx) - `processBarePagePatterns` 함수

#### 🗂️ .gitignore 파일 개선
- [x] **카테고리별 구조화**: Dependencies, Build outputs, Environment, IDE, OS, Logs, Cache, Temporary files
- [x] **가독성 향상**: 섹션 주석으로 명확한 구분
- [x] **패턴 확장**:
  - 캐시 폴더: `.cache/`, `.parcel-cache/`
  - 환경 변수: `.env.*.local` 패턴
  - IDE 파일: `.swp`, `.swo`, `*~`
  - 임시 파일: `*.tmp`, `*.temp`

**구현 위치**: [.gitignore](/.gitignore)

---

### 🎨 2026-01-02: UI/UX 개선 + 성능 최적화 + 기본 설정 변경

#### 🔧 무한 루프 버그 수정
- [x] **문제**: `useEffect`가 함수 참조 변경을 감지하여 무한 렌더링 루프 발생
- [x] **해결**: `handleChatUpdate`와 `handlePageClick`을 `useCallback`으로 메모이제이션
- [x] **결과**: "Maximum update depth exceeded" 에러 해결, 성능 향상

**구현 위치**: 
- [App.jsx:107-116](src/App.jsx) - `handleChatUpdate` useCallback
- [App.jsx:119-139](src/App.jsx) - `handlePageClick` useCallback
- [ChatInterface.jsx:119-123](src/components/ChatInterface.jsx) - useEffect dependency 수정

#### 📱 스튜디오 패널 UI 최적화
- [x] **문서 메타데이터 섹션 제거**: 파일명, 문자 수, 페이지 수, 타입 정보 섹션 완전 제거
- [x] **컴팩트 디자인**: 모든 섹션 패딩 축소 (p-6 → p-4)
  - 핵심 요약: 아이콘 크기 축소 (w-8 h-8 → w-6 h-6), 텍스트 영역 최적화
  - 주요 내용: 리스트 간격 축소 (space-y-3 → space-y-2), 숫자 배지 축소
  - 핵심 키워드: 마진 최적화 (mb-3 → mb-2)
- [x] **가독성 향상**: 본문 텍스트 크기 조정 (text-xs → text-sm)
- [x] **결과**: 스크롤 없이 한 화면에 모든 인터페이스 표시 가능

**구현 위치**: [DataPreview.jsx:1289-1480](src/components/DataPreview.jsx)

#### ⚙️ 기본 설정 변경
- [x] **기본 AI 모드**: 'thinking' → 'instant' (빠름 모드)
- [x] **스튜디오 초기 모드**: 'pdf' → 'natural' (자연어 분석 모드)
  - 인용 배지 클릭 시 PDF 뷰어 모드로 자동 전환

**구현 위치**:
- [App.jsx:12](src/App.jsx) - selectedModel 기본값
- [DataPreview.jsx:110](src/components/DataPreview.jsx) - viewMode 기본값

#### 🔗 인용 배지 클릭 이벤트 개선
- [x] **문제**: `renderTextWithCitations` 함수가 `onPageClick` 핸들러를 제대로 전달하지 않음
- [x] **해결**: 함수 파라미터에 `pageClickHandler` 추가 및 모든 호출부에서 명시적 전달
- [x] **React.Fragment 적용**: 배열 반환 시 Fragment로 감싸서 ReactMarkdown과 호환성 향상

**구현 위치**: [ChatInterface.jsx:20-108](src/components/ChatInterface.jsx) - `renderTextWithCitations` 함수

### 🎯 2026-01-02: AI 지침 설정 + 복합 인용 배지 + PDF 스크롤 안정화

#### ⚙️ AI 행동 지침 설정 패널 (SystemPromptPanel)
- [x] **우측 패널 하단 페르소나 설정 UI**: 자연어 인사이트 카드 아래에 통합
- [x] **프리셋 페르소나 버튼**:
  - "에이비딩 운영자": 입찰 프로세스, 계약 관리 전문
  - "일반 문서 분석가": 중립적 문서 분석
- [x] **커스텀 지침 입력**: Textarea로 자유롭게 AI 역할, 응답 스타일 설정
- [x] **적용/초기화 버튼**: 실시간 AI 지침 적용 및 리셋
- [x] **보라색 그라데이션 디자인**: NotebookLM 스타일 일관성 유지

**사용 예시**:
```
프리셋 선택 → "에이비딩 운영자" 클릭 → 적용 버튼
→ AI가 법률 규정 인용 스타일로 응답 시작
```

**구현 위치**: [SystemPromptPanel.jsx](src/components/SystemPromptPanel.jsx)

#### 🏷️ 복합 인용 배지 파싱 강화
- [x] **기존 제한**: 숫자만 인식 (`[5]`, `[5-8]`, `[5, 10]`)
- [x] **개선 후**: 숫자 + 텍스트 혼합 패턴 지원
  - `[35, 38, 문서 맥락 기반 추론]` → 파란 배지 2개 + 보라 배지 1개
  - `[5, 10, 추가 분석 필요]` → 파란 배지 2개 + 보라 배지 1개

**배지 타입**:
- **파란색 페이지 배지**: `[5]`, `[5-8]` (FileText 아이콘)
- **보라색 추론 배지**: `[💡 문서 맥락 기반 추론]` (Lightbulb 아이콘)

**정규식 변경**:
```javascript
// Before: /\[\s*(\d+(?:\s*-\s*\d+)?(?:\s*,\s*\d+(?:\s*-\s*\d+)?)*)\s*\]/g
// After: /\[([^\]]+)\]/g  // 모든 내용 캡처 후 타입 분류
```

**구현 위치**: [ChatInterface.jsx](src/components/ChatInterface.jsx) - `renderTextWithCitations` 함수

#### 🎯 PDF 스크롤 안정화 (Retry 로직)
- [x] **기존 문제**: 배지 클릭 시 콘솔 로그만 출력, 페이지 이동 실패
- [x] **원인 1**: `App.jsx`에서 PDF 모드로 전환하지 않음
- [x] **원인 2**: 고정 350ms 지연으로 느린 렌더링 시 실패

**해결책**:
1. **App.jsx**: 배지 클릭 시 즉시 PDF 모드 전환
   ```javascript
   setRightPanelState({ mode: 'pdf', pdfPage: pageNumber })
   ```

2. **DataPreview.jsx**: 재시도 로직으로 DOM 대기
   ```javascript
   // 최대 5회 재시도 (100ms → 200ms → 300ms → 400ms → 500ms)
   const tryScroll = (attempt = 1, maxAttempts = 5) => {
     if (pageElement) {
       scrollTo({ top: ..., behavior: 'smooth' })  // 즉시 성공
     } else if (attempt < maxAttempts) {
       setTimeout(() => tryScroll(attempt + 1), 100 * attempt)  // 재귀 재시도
     }
   }
   ```

**성능 개선**:
- **PDF 모드 → 배지 클릭**: 50ms 내 이동 ✅
- **자연어 모드 → 배지 클릭**: 200-400ms 내 이동 ✅
- **느린 렌더링 (500ms)**: 기존 실패 → 성공 ✅

**구현 위치**:
- [App.jsx:118-139](src/App.jsx) - `handlePageClick` 함수
- [DataPreview.jsx:527-585](src/components/DataPreview.jsx) - `handlePageNavigate` 함수

#### 📝 Markdown 리스트 내 인용 배지 렌더링
- [x] **기존 문제**: `1. **굵게** [5]` → 배지 변환 안 됨 (React 요소 트리로 전달)
- [x] **해결**: 재귀 React 요소 순회로 문자열 찾아 배지 변환

**React 요소 트리 예시**:
```jsx
<li>
  <strong>첫 번째 항목</strong>  // React 요소
  "은 [5]에 있습니다."          // 문자열 (배지 변환 대상)
</li>
```
    ...
    return React.Children.map(nodes, (child) => {
      ...
      if (React.isValidElement(child) && child.props.children) {
        ...
        return React.cloneElement(child, {
          children: processNodes(child.props.children)  // 재귀!
        })
      }
      return child
    })
  }
  return <li>{processNodes(children)}</li>
}
```

**처리 가능한 케이스**:
- ✅ `1. **굵게** [5]` → **굵게** + 배지
- ✅ `1. ***매우 굵게*** [5, 10]` → ***매우 굵게*** + 배지 2개
- ✅ `1. \`코드\` [15]` → `코드` + 배지
- ✅ 중첩 리스트 + 인용

**구현 위치**: [ChatInterface.jsx:476-522](src/components/ChatInterface.jsx) - ReactMarkdown `li` 컴포넌트

#### 📚 관련 문서
- [AI_GUIDELINES_USER_GUIDE.md](AI_GUIDELINES_USER_GUIDE.md) - AI 지침 설정 사용 가이드
- [CITATION_REFACTORING.md](CITATION_REFACTORING.md) - 복합 인용 배지 파싱 로직
- [SCROLL_FIX.md](SCROLL_FIX.md) - PDF 스크롤 Retry 시스템
- [MARKDOWN_LIST_CITATION_FIX.md](MARKDOWN_LIST_CITATION_FIX.md) - 재귀 React 요소 처리
