...
- **마지막 업데이트**: 2026-01-07 (다중 파일 인용 시스템 완전 수정)

## ✅ 완료된 기능 (2026-01-06 업데이트)
...
- [x] **다중 파일 인용 정확성 (2026-01-07 수정)**:
  - 누적 페이지 번호 시스템 구현
  - 각 파일별 로컬 페이지 번호 자동 변환
  - 인용 클릭 시 정확한 소스 파일 열림
...
### 최근 결정 사항
- 다중 파일 인용 시스템 완전 수정 (2026-01-07)
  - **문제**: 2개 이상 파일 선택 시 모든 인용이 첫 번째 파일로 연결되는 버그
  - **해결**: 누적 페이지 번호 시스템 + findFileByPageNumber 함수
    - 파일 1 (30페이지): 전역 페이지 1-30 → 로컬 페이지 1-30
    - 파일 2 (20페이지): 전역 페이지 31-50 → 로컬 페이지 1-20
  - **구현 파일**:
    - ChatInterface.jsx: allSourcesData 생성 (누적 페이지 범위 계산)
    - ChatInterface.jsx: findFileByPageNumber 헬퍼 함수
    - App.jsx: handlePageClick에서 targetFile 찾기 및 localPageNumber 변환
    - App.jsx: DataPreview에 `rightPanelState.targetFile` 전달
